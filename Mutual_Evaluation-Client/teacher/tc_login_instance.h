...
//单例模式，主要保存当前登录教师用户，服务器信息
class Tc_Login_Instance
{
public:
     static Tc_Login_Instance *getInstance(); //保证唯一一个实例
...
private:
    ...
    Tc_Login_Instance(const Tc_Login_Instance&);
    Tc_Login_Instance& operator=(const Tc_Login_Instance&);
    ...
    //静态数据成员，类中声明，类外必须定义
    static Tc_Login_Instance *m_instance;
