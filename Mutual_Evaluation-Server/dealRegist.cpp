 ...
 */
void DealRegist::read_regist_messages(QByteArray send_buf)
{
    ...
    QString phone;
    QString email;
    get_regist_info(reg_buf,user,nick_name,pwd,phone,email);

    QString msg; //用于判断是否注册成功
    if(!this->m_mysql.user_is_exist(user)) {
        msg = "true";   //用户不存在,可以进行注册
        m_mysql.regist_success(user,nick_name,pwd,phone,email);//注册个人信息到数据库
    } else {
 ...
 */
void DealRegist::get_regist_info(QByteArray reg_buf,QString &user, QString &nick_name, QString &pwd, QString &tel, QString &email)
{
        /*json数据如下
        {
            userName:xxxx,
