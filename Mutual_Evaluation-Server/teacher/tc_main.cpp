 ...
 */
void Tc_Main::read_module_messages(QByteArray send_buf)
{
    ...
    /*请求哪个模块的处理*/
    if(module == "deal_tc_info"){                            //教师信息处理模块
        Deal_Tc_Info deal_tc_info{m_tcpSocket, m_info_mysql}; //教师信息相关处理对象
        deal_tc_info.read_service_messages(send_buf);
    }
...
/**
 * @brief Tc_Main::get_module_json 解析客户端请求服务的json数据包
 * @param temp_buf, module... 缓冲区，应该是处理学生请求的哪个模块
 */
void Tc_Main::get_module_json(QByteArray temp_buf, QString &module)
{        /*json数据如下
        {
            sender:xxxx,
            module:xxx,
